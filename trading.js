...
const {
  ...
  DAILY_MAX_LOSSES, RECOVERY_TRIGGER_LOSSES, RECOVERY_MIN_RR,
} = require('./config');
...
async function applyLotSizeFilter(client, symbol, rawQty, entry) {
  try {
    ...
    let stepSize = null, minQty = null, minNotional = null;
    for (const f of (symInfo.filters || [])) {
      if (f.filterType === 'LOT_SIZE') {
        stepSize = parseFloat(f.stepSize); minQty = parseFloat(f.minQty);
      }
    ...
    if (stepSize && stepSize > 0) {
      ...
      const dec = stepSize.toString().includes('.') ? stepSize.toString().split('.')[1].length : 0;
      qty = parseFloat(qty.toFixed(dec));
    }
    if (minQty && qty < minQty) return 0;
    if (minNotional && qty * entry < minNotional) return 0;
    return qty;
  } catch { return rawQty; }
}
...
async function checkDailyProtection(user) {
  ...
  if (fresh.daily_losses >= DAILY_MAX_LOSSES) {
    if (!fresh.recovery_mode) await db.users.update(user.telegram_id, { recovery_mode: true });
    return { blocked: false, recovery: true };
  }
...
async function openTrade(user, signal) {
  // Lock key includes market_type to allow same symbol on different markets independently
  ...
  return withTradeLock(lockKey, async () => {
    try {
      ...
      const fresh = db.users.findById(user.telegram_id);
      const inRecovery = fresh?.recovery_mode || false;
      if (inRecovery && signal.score < 95)           return { success: false, reason: 'RECOVERY_SCORE_TOO_LOW' };
      if (inRecovery && signal.rr < RECOVERY_MIN_RR) return { success: false, reason: 'RECOVERY_RR_INSUFFICIENT' };
      ...
      if (balData.free < 10) return { success: false, reason: 'INSUFFICIENT_BALANCE' };

      const riskPct  = getRiskPct(signal.score, inRecovery);
      const rawQty   = calculatePositionSize(balData.free, riskPct, signal.entry, signal.sl);
      ...
      const leverage = 5;
      const margin_used = user.market_type === 'futures' ? parseFloat(((quantity * filledPrice) / leverage).toFixed(4)) : 0;
...
async function clearUserAccountData(userId) {
  ...
  // Close all open trades for this user (they belong to the old account)
...
async function _updateUserStatsOnClose(user, profit, profitPct, result_label) {
  try {
    ...
    const exitedRecovery = fresh.recovery_mode && isWin;
    ...
    await db.users.update(user.telegram_id, {
      ...
      avg_win:            avgWin,
      avg_loss:           avgLoss,
      ...(exitedRecovery ? { recovery_mode: false, consecutive_losses: 0 } : {}),
    });
  } catch (err) {
